...
public class DefenseUpPotion : Item
{
    public float defenseUpDuration = 100f;
    public float defenseUpPercent = 0.2f;
    ...
    public override void OnUse(Character character)
    {
        ...
        {
            player.ApplyBuff(BuffType.DefenseUp, defenseUpDuration, defenseUpPercent);
            Debug.Log($"[Potion] {player.name} used defence Up Potion! defence increased by {defenseUpPercent * 100}% for {defenseUpDuration} seconds.");
        }
        quantity--;
    }
